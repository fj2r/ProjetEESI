...
def deplacementPerso(keyb, x, y, direction,index ):
    if keyb[pg.K_LEFT]:
        direction= pg.K_LEFT
        index = (index+1)%4
        x=x-keyb[pg.K_LEFT]*8
    if keyb[pg.K_RIGHT]:
        direction= pg.K_RIGHT
        index = (index+1)%4
        x=x+keyb[pg.K_RIGHT]*8
    if keyb[pg.K_DOWN]:
        direction= pg.K_DOWN
        index = (index+1)%4
        y=y+keyb[pg.K_DOWN]*8
    if keyb[pg.K_UP]:
        direction= pg.K_UP
        index = (index+1)%4
        y=y-keyb[pg.K_UP]*8
...
def main() :
    ...
    while jeu:
        ...
        if etat == 1 :
            ...
            etatSprite = deplacementPerso(keypressed,x_sprite,y_sprite,direction,index_sprite)
            print(etatSprite)
